...
import { enrichDealCards } from '@/lib/db/deal-index.repo';
 ...
 *  flows through, so one batch price-history read here (Deal Index + real
 *  cardiogram) covers home/category/search without per-page plumbing. */
export async function DealGrid({ deals, listName = 'grid' }: { deals: NormalizedDeal[]; listName?: string }) {
  const enriched = await enrichDealCards(deals);
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      <TrackViewItemList listName={listName} items={enriched.map(gaItem)} />
      {enriched.map((deal, i) => (
