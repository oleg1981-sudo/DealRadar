# Product Requirements — Version 3 (Platform Scope) — DRAFT
...
* **Version:** v3.0. **Supersedes** the `2026-06-23_v2/` ground-source-of-truth suite **and** folds in the `2026-06-28` Tier-1 remediation plan (`docs/remediation_plan/2026-06-28_v1/`). Where v2 self-reported "all findings RESOLVED," this v3 honors the **code-grounded** status instead.
* **Date:** 2026-07-08
* **Sibling docs:** [requirements.md](file:///Users/danielmanzela/DealRadar/docs/ground_source_of_truth/2026-07-08_v3/requirements.md) (`FR-*`/`NFR-*`), [design.md](file:///Users/danielmanzela/DealRadar/docs/ground_source_of_truth/2026-07-08_v3/design.md) (`DSN-*`), [tasks.md](file:///Users/danielmanzela/DealRadar/docs/ground_source_of_truth/2026-07-08_v3/tasks.md) (`T-*`).
...
|---|---|---|
| Zero indexed URLs | GSC coverage = 0; prod build lags branch | No organic traffic → no clicks → **no commission** |
| Prod build lags branch | Prod = `dealradar.me`, deployed SHA ≠ `feat/tier1-remediation-2026-06-28` HEAD | Remediations are invisible to users/crawlers |
| Stale host strings | `BASE_URL`/`robots`/`imprint` fall back to `dealradar.eu` | Canonical/hreflang/JSON-LD point at the wrong host → index poisoning |
...
| Thin PDP content | PDP renders name/price/shop/image/history only; `product-details.ts` dead | Weak AEO/GEO extraction; thin-content risk at scale |
...
| **A-02** | The `feat/tier1-remediation-2026-06-28` branch is the intended prod build to deploy. | Ops | branch = deploy source |
...
| **A-19** | The APPROVED, in-flight **url-slug rework** (public_id-stable URLs, redirect ladder, expired-state, sitemap sharding, category metadata, `/search` noindex) is the prerequisite implementation vehicle for M2 scale. Its authoritative acceptance in v3 is `DSN-SEO-1/4/5/6/7` (design.md §4). | Growth | M2 prerequisite |
...
* **Evaluate:** `count(*) FROM deals WHERE source<>'mock' > 0` for live countries; 0 rows with `discount_percent NOT BETWEEN 0 AND 100`; 0 `slug IS NULL`; data staleness ≤ 26 h.
* **Verify:** integration refresh logs `upserted N>0` + dedup ratio; `registry.test.ts` green.
* **Monitor:** per-source `upserted`, dedup-ratio drift, `max(now − last_updated)`.
...
| **SC5** | Google Rich Results Test = **0 errors** on live PDPs across ≥ 3 locales. | M1 |
...
## 8. Risks (RSK-1 … RSK-12), Sequencing & Operator Thresholds
...
| **RSK-2** | Stale `dealradar.eu`/`.app` host poisons canonical/hreflang/JSON-LD. | Env pin + build-failing grep gate. | T-SEO-1 |
...
| **RSK-5** | Thin PDP content → thin-content penalty / weak AEO/GEO extraction. | Editorial overlay + non-thin blocks with a legal gate. | T-PDP-3/4 |
...
_End of v3 PRD. This document defines OBJ-1…OBJ-8, Assumptions A-01…A-19, Success Criteria SC1…SC8, Risks RSK-1…RSK-12, §6.3 payout scope, and §8.2 Operator thresholds that requirements.md, design.md, and tasks.md cite. Never promise a literal 100% (§1.2)._
