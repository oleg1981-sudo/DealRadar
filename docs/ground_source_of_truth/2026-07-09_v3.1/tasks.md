...
- [ ] T-INF-1: Apply the remediation + on-disk schema migration to the **prod** Supabase DB (A-07) via the allow-listed idempotent runner. _**DB half DONE 2026-07-10** — applied via Supabase MCP `apply_migration` as `v3_1_remediation_layer_baseline` (starts migration-history tracking; `schema_migrations` row recorded). Pre-flight: server-side snapshot `backup_20260710` (835/8/2406 rows, drop after T-INF-2 verifies); slug-collision pre-check = 0/835. Post-apply contract test (`scripts/db-verify.mjs` tiers, run via MCP): **shape 15/15, semantics 6/6 (slug auto-compose, one-snapshot-per-day conflict-update, non-price patch no-fire, tx idempotency, RPC), retrieval 8/8, zero probe residue** — and **802** deals carry `historical_low_price` (an earlier 803 count transiently included the synthetic probe deal; corrected after fresh post-cleanup verification 2026-07-10). Deployed-build compatibility **verified-live** post-apply: `dealradar.me/api/deals` → 200 with 24 deals (`cached:false` — a fresh DB read), homepage + search → 200. **⛔ Directive: run `/agent-skills:ship` BEFORE executing T-INF-2.** **Keep unticked:** the deploy-facing clauses (prod curl via `db-migrate.yml` path, re-apply-idempotency in CI) close with T-INF-2, which is now UNBLOCKED._
...
- [x] T-INF-2: Deploy the `feat/tier1-remediation-2026-06-28` build (branch HEAD — `b53bb3e` at authoring; `155cf06` is the code tree) to prod Netlify — close the merged≠deployed gap. _**EXECUTED + VERIFIED-LIVE 2026-07-10.** Pre-deploy `/agent-skills:ship` gate ran per directive (3-persona fan-out): initial NO-GO with 4 blockers (`.eu` fallback poisoning canonicals/sitemap/unsubscribe; `deal.priceNote`+6 keys missing from all 13 locales; hidden-deal leak in 3 repo fns; unescaped JSON-LD) — all fixed + 7 recommended (sitemap `revalidate=3600`, `.range()` pagination past the 1000-row PostgREST cap, `.in()` chunking, search `noindex`, CSP/HSTS headers, postbacks GET, 12 new revenue/PII tests → 40/40) + risks (dead `DealDetailModal`/`unsubscribe.ts` deleted, refresh count race, `Offer` not `AggregateOffer`, imprint `.me`) in `5218271`, merged `56dea25`. **Root cause of a ~1h stall: Netlify deploys from UPSTREAM `oleg1981-sudo/DealRadar`, NOT the `Manzela` fork** — pushes to the fork never deploy; resolved by `git push upstream main` (user-authorized, 19 commits). Also shipped: `fb82d58` (SonarCloud gate C→A: `Math.random`→CSPRNG `rng.ts` ×4, removed `key-hero-bg.cjs` ×2), `320e233` (schema.sql `search_path` pins). Live acceptance via `scripts/verify-deploy.mjs`: **16/17 PASS** (802 deal URLs in sitemap, 0 `.eu`, PDP 200 + `.me` canonical + escaped single-`Offer` JSON-LD, CSP/HSTS/DENY headers, search `noindex`, no raw i18n key). 17th (robots.txt sitemap host — Netlify `URL`=`dealradar.eu` is the PRIMARY domain; fix `569303b` routes it through `siteUrl()`) pushed, Netlify build stalled — needs dashboard publish. DB `search_path` hardening applied via SQL editor + verified (`proconfig` ×6, rolled-back semantics probe green). `backup_20260710` drop still pending (user SQL block #2)._
