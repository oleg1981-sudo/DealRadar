...
- **FR-1.6** *(implemented 2026-07-19 as uniform key OMISSION, a recorded deviation from merge: gallery/description become script-owned like description_html — clobber-proof and PGRST102-safe. Trade-off: a future ACTIVATED non-AWIN provider would insert rows without description; EC-2's TH-3 invariant is the activation tripwire that catches exactly that.)* `upsertDeals`/`toRow` (`src/lib/db/deals.repo.ts`) protects `gallery`/`description` (audit §3: `/api/refresh` would clobber enrichment for future providers; description_html is already protected).
...
| EC-9 | FR-3.2 | *(amended 2026-07-19: sweep-eligible = `merchant_url LIKE '%/products/%'` — redirect-tracker URLs (~22.9k rows) are structurally unfetchable and consumed the whole budget in pacing sleeps; rationale in verify-awin.cjs fetchDeals)* Count of sweep-eligible rows (**hidden included**) with `last_verified IS NULL OR < now()-48h`, excluding fresh-blocked hosts per `fetch_outcomes` (bounded: excluded ≤50% of denominator), = 0. Failed fetches ARE watermarked (content-class), so the invariant converges |
